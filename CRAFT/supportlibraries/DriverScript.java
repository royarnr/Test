...
public class DriverScript {
	...
	private final FrameworkParameters frameworkParameters = FrameworkParameters
			.getInstance();
	public static boolean uploadToQCAllAtOnce = false;
	...
	public void driveTestExecution() {
		...
		/*
		 * Added for ALM Connection
		 */
	
		
		// If the QcOnOff Flag is ON then it will update the result in QC
		if(properties.getProperty("QCOnOff").equalsIgnoreCase("On"))
		{
			UpdateQCTestResult();
			// After result update it will disconnect with QC
//			System.out.println("Disconnecting with QC....");
//			QCLoginObj.disconnect();
		}
	}
	...
	private void initializeWebDriver() {
		...
		case PERFECTO:
			...
			if (testParameters.getMobileToolName().equals(
				...
				/*
				 * *************************************
				 * *********Changed as a part of Persona Implementation***
				 */
				WebDriver remotePerfectoDriver = PerfectoDriverFactory
						.getPerfectoRemoteWebDriver(
								...
								testParameters.getDeviceName(),
								mobileProperties.getProperty("PerfectoHost"),
								testParameters.getBrowser(), testParameters.getPersona());
	...
	private void executeCRAFTTestIterations() {
		while (currentIteration <= testParameters.getEndIteration()) {
			...
			/*
			 * Code added for stackoverflow error
			 */
			catch (StackOverflowError stck)
			{
				errorHandler(stck, "Error");
			}
	...
	private void errorHandler(Error ex, String exceptionName) {
		// Error reporting
		String exceptionDescription = ex.getMessage();
		if (exceptionDescription == null) {
			exceptionDescription = ex.toString();
	...
	/*
	 * This function will be uploading the test case result along with the images to QC Test Run
	 */
	public void UpdateQCTestResult(){
		
		//Getting the current Test case name from TestParameter Class
		//String CurrentTest = TestParameters.currentScenario + "_" + TestParameters.currentTestcase + "_" + TestParameters.currentTestInstance;
		String CurrentTest = testParameters.getCurrentScenario()+ "_" + testParameters.getCurrentTestcase() + "_" + testParameters.getCurrentTestInstance();
		//Creating an array list to add HTML file and image file for upload
		List<File> attach = new ArrayList<>();
	    attach.add(new File(resultSummaryManager.reportPath + File.separator + "HTML Results", CurrentTest + ".html"));
	    
	    //Adding the list of images for current test case only. Else for every run all image will get uploaded
	    File imgFolder = new File(resultSummaryManager.reportPath + File.separator + "Screenshots");
	    if (imgFolder.exists()) {
	        for (File image : imgFolder.listFiles()) {
	            if (image.getName().startsWith(CurrentTest)) {
	                attach.add(image);
	            }
	        }
	    }
